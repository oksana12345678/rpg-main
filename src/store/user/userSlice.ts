...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    setQuestStatus: (
      ...
      if (state.user) {
        const quest = state.user.userQuests.find((q) => q.quest.name === action.payload.name);
        if (quest) {
          quest.status = action.payload.status;
          quest.isLocked = action.payload.isLocked;
          quest.progress = action.payload.progress;
          quest.home_work_link = action.payload.home_work_link;
        }
      }
    },
  },
