...
const QuestDetailsPageV2 = () => {
  ...
  useEffect(() => {
    dispatch(questsApi.endpoints.getQuestsTask.initiate({}));
  }, [dispatch]);
  ...
  const getRouteForQuest = (questName: string): string | null => {
    return questName === 'Базовий квест' ? ROUTES.CHOOSE_CLASS : ROUTES.QUESTS_TASK;
  };
  ...
  return (
    <main className="flex min-h-screen w-[100%] items-center justify-center bg-bg_text_accent py-6 text-white">
      <div className="flex max-w-[500px] flex-col gap-5 px-6">
        ...
        <BaseShadowContainer
          className="flex max-h-40 flex-col items-center justify-center gap-6 overflow-scroll rounded-lg px-5 py-3 shadow-shadow_text_accent"
          contentStyle="text-sm leading-5"
