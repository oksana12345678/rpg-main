...
const QuestCompletePage = () => {
  ...
  const selectedDetails = questDetails?.complete_quest_regards?.find(
    ...
    try {
      ...
      const updatedActiveQuests = currentActiveQuests.map((quest) =>
        quest.quest.name === questDetails.name
          ? { ...quest, name: questDetails.name, status: 'complete', isLocked: true, progress: 100 }
          : quest,
      );
      await updateUserAccount({
        coins: (user?.coins ?? 0) + selectedDetails.coins,
        userClass: selectedDetails.name ?? user?.userClass,
